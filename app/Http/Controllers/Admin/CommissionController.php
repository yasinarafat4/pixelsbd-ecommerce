...
class CommissionController extends Controller
{
    ...
    public function pay_to_seller(Request $request)
    {
        ...
        if ($request->payment_option == 'cash') {
            ...
            $payment = new Payment;
            $payment->seller_id = $shop->seller->id;
            ...
            flash_success(trans('Payment completed'));
    ...
    public function seller_payment_done($payment_data, $payment_details)
    {
        ...
        $users = Seller::findMany($shop->seller->id);
        $data = array();
        $data['user'] = $shop->seller;
        ...
        if ($payment_data['payment_withdraw'] == 'withdraw_request') {
            flash_success(trans('Payment completed'));
            return redirect()->route('admin.seller.payout_requests');
        } else {
            flash_success(trans('Payment completed'));
    ...
    public function calculateCommission($order)
    {
        if ($order->payment_type == 'cash_on_delivery') {
            foreach ($order->orderDetails as $orderDetail) {
                ...
                if (get_settings('vendor_commission_activation')) {
                    if (get_settings('category_wise_commission')) {
                        $commission_percentage = $orderDetail->product->category->commision_rate;
                    } else if ($orderDetail->product->user->user_type == 'seller') {
                        $commission_percentage = get_settings('vendor_commission');
                    }
                }
                if ($orderDetail->product->user->user_type == 'seller') {
                    ...
                    if (get_settings('product_manage_by_admin') == 1) {
