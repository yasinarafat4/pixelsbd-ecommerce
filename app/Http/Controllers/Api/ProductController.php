...
use App\Http\Resources\Api\ProductDetailCollection;
...
class ProductController extends Controller
{
        ...
        return response()->json(
            ...
            [
                ...
                'data' => [
                    ...
                    'max_limit' => $max_limit,
                    'in_stock' => $in_stock,
                    'image' => $product_stock->image == null ? "" : static_asset($product_stock->image)
                ]
    ...
    public function newArrivals()
    {
        ...
        return new ProductMiniCollection(filter_products($products)->latest()->limit(12)->get());
    }

    public function topRated()
    {
        $products = Product::orderBy('rating', 'desc')->physical();
        return new ProductMiniCollection(filter_products($products)->limit(8)->get());
    }
    ...
    public function more_store_items($product_id, $user_id, $user_type)
    {
        $products = Product::whereNot('id', $product_id)->where('user_id', $user_id)->where('added_by', $user_type);
        return new ProductMiniCollection(filter_products($products)->limit(8)->get());
    }

    public function similar_products($product_id, $category_id)
    {
        $category = Category::find($category_id);
        $categories = $category->descendants()->pluck('id');
        $categories[] = $category->getKey();
        $similar_products = Product::whereNot('id', $product_id)->whereIn('category_id', $categories);
        return new ProductMiniCollection(filter_products($similar_products)->limit(5)->get());
    }
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($category_ids)) {
            ...
            foreach ($category_ids as $cid) {
                $n_cid = array_merge($n_cid, Category::whereDescendantOf($cid)->pluck('id'));
            }
