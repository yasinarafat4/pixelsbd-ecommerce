...
class NotificationUtility
{
    public static function sendOrderPlacedNotification($order, $request = null)
    {
        ...
        if ($request != null && get_settings('google_firebase') == 1 && $order->user->device_token != null) {
    ...
    public static function sendNotification($order, $order_status)
    {
        ...
        if ($order->orderDetails->first()->product->user->user_type != 'admin') {
            $seller = \App\Models\Seller::find($order->seller->id);
            array_push($users, $seller);
        }
        $customer = \App\Models\User::find($order->user->id);
