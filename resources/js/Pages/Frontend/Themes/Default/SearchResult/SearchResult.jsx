...
export default function SearchResult({ label, products, minimum_price, maximum_price, colors }) {
    ...
    function onSortByChange(e) {
        ...
        router.visit(route('search', [{ 'search': search, 'sort_by': e.target.value, 'min_price': min_price, 'max_price': max_price, 'color': color_name }]))
    }

    // filter by price range
    function onInput(e) {
        setRangeValue(e)
        router.visit(route('search', [{ 'search': search, 'sort_by': sort_by, 'min_price': e[0], 'max_price': e[1], 'color': color_name }]))
    }
    ...
    function onColorClick(color) {
        if (color_name == color) {
            setSelectedColor('');
            router.visit(route('search', [{ 'search': search, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price }]))
        } else {
            setSelectedColor(color);
            router.visit(route('search', [{ 'search': search, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price, 'color': color }]))
        }
    }
                    ...
                    <div className='lg:col-span-8 xl:col-span-9 space-y-3 md:space-y-0 text-slate-600'>
                        ...
                        <div className="lg:hidden flex flex-col w-full gap-4 justify-between items-center py-2 border-b">
                            <div className="w-full">
                                <h2 className="font-semibold text-lg w-full">{label}<span className="font-samibold text-slate-500"> {search}</span></h2>
                        ...
                        <div className="hidden w-full lg:flex flex-col lg:flex-row gap-2 justify-between items-center border-b pb-1">
                            <div className="w-full">
                                <h2 className="font-semibold text-2xl w-full">{label}<span className="font-samibold text-slate-500"> {search}</span></h2>
                        ...
                        </div> : <NothingFound title={'No Search Result Found!'} />}
                    </div>
                </div>
            </div>
        </DefaultThemeLayout>
