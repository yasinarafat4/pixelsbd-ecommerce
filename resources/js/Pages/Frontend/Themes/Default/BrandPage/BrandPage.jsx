...
export default function BrandPage({ brand, products, minimum_price, maximum_price, colors }) {
    ...
    function onSortByChange(e) {
        ...
        router.visit(route('brand_page', { 'slug': brand.slug, 'sort_by': e.target.value, 'min_price': min_price, 'max_price': max_price, 'color': color_name }))
    }

    // filter by price range
    function onInput(e) {
        setRangeValue(e)
        router.visit(route('brand_page', { 'slug': brand.slug, 'sort_by': sort_by, 'min_price': e[0], 'max_price': e[1], 'color': color_name }))
    }
    ...
    function onColorClick(color) {
        if (color_name == color) {
            setSelectedColor('');
            router.visit(route('brand_page', { 'slug': brand.slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price }))
        } else {
            setSelectedColor(color);
            router.visit(route('brand_page', { 'slug': brand.slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price, 'color': color }))
        }
    }

    return (
        <DefaultThemeLayout>
            <Head title={"Brand Page"} />
            <div>
                {/* Body */}
                <div className='relative lg:grid lg:grid-cols-11 xl:grid-cols-12 gap-3 xl:gap-7 lg:my-5 text-slate-600'>
                    ...
                    {/* Products */}
                    <div className='lg:col-span-8 xl:col-span-9 space-y-1 text-slate-600'>
                        {/* Sorting for Small Devices */}
                        <div className="lg:hidden flex flex-col w-full gap-2 justify-between items-center py-2 border-b">
                            <div className="w-full">
                                <h2 className="font-semibold text-lg w-full">{'Brand Products'} ({brand.name})</h2>
                        ...
                        <div className="hidden w-full lg:flex flex-col lg:flex-row gap-2 justify-between items-center border-b pb-1">
                            <div className="w-full">
                                <h2 className="font-semibold text-2xl w-full">{'Brand Products'} ({brand.name})</h2>
