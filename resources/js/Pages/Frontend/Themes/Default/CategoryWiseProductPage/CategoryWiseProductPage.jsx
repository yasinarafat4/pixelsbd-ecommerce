...
import { asset_url, currencyFormat, placeholder1_1, placeholder6_1 } from "@/Helpers";
...
export default function CategoryWiseProductPage({ colors, breadcrumbs, childcategories, products, minimum_price, maximum_price }) {
    ...
    function onSortByChange(e) {
        ...
        router.visit(route('catgeory_wise_product', { 'slug': childcategories.slug, 'sort_by': e.target.value, 'min_price': min_price, 'max_price': max_price, 'color': color_name }))
    }

    // filter by price range
    function onInput(e) {
        setRangeValue(e)
        router.visit(route('catgeory_wise_product', { 'slug': childcategories.slug, 'sort_by': sort_by, 'min_price': e[0], 'max_price': e[1], 'color': color_name }))
    }
    ...
    function onColorClick(color) {
        if (color_name == color) {
            setSelectedColor('');
            router.visit(route('catgeory_wise_product', { 'slug': childcategories.slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price }))
        } else {
            setSelectedColor(color);
            router.visit(route('catgeory_wise_product', { 'slug': childcategories.slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price, 'color': color }))
        }
    }
    ...
    return (
        <DefaultThemeLayout>
            <Head title={"Category Wise Product"} />
            <div>
                {/* Breadcrumbs */}
                <div  className="text-sm breadcrumbs text-slate-600">
                    <ul>
                        ...
                        {breadcrumbs.map((breadcrumb, i) => (
                            <li key={i}>
                                <Link href={route('catgeory_wise_product', breadcrumb.slug)} key={i}>
                ...
                {/* Body */}
                <div  className='relative lg:grid grid-cols-11 xl:grid-cols-12 gap-3 xl:gap-6 text-slate-600 lg:my-5'>
                    ...
                    <div  className='hidden lg:block lg:col-span-3 xl:col-span-3 space-y-5'>
                        ...
                        <details open tabIndex={0}  className="collapse collapse-open collapse-arrow cursor-pointer bg-white border border-slate-300 p-2 rounded-sm">
                            ...
                            <div  className="collapse-content shadow-lg">
                                {
                                    childcategories.descendants.map((category, i) => (
                                        <Link href={route('catgeory_wise_product', category.slug)} key={i}>
                                            <div  className="border-1 border-b px-2 py-[8px] mr-3 border-slate-300 text_primary hover:bg_soft_primary duration-500 text-[14px] flex justify-between items-center">
                                                <div  className='flex items-center gap-2'>
                                                    <div  className='w-6 h-6'>
                                                        <LazyLoadImage
                                                             className='w-full h-full pointer-events-none mix-blend-multiply'
                                                            src={asset_url(category.icon || placeholder1_1())}
                                                            alt={category.name}
                                                            effect='blur'
                                                        />
                                                    </div>
                        ...
                        <div  className="bg-white border border-slate-300 py-4 px-4 xl:px-6 space-y-7 rounded-sm">
                            ...
                            <RangeSlider onInput={e => onInput(e)} defaultValue={rangeValue} min={minimum_price} max={maximum_price} id="range-slider-yellow" />
                                        ...
                                        <div  className="collapse-content shadow-lg">
                                            {
                                                childcategories.descendants.map((category, i) => (
                                                    <Link href={route('catgeory_wise_product', category.slug)} key={i}>
                                                        <div  className="border-1 border-b px-1 py-[8px] mr-1 border-slate-300 text_primary hover:bg_soft_primary duration-500 text-[11px] md:text-[14px] flex justify-between items-center">
                                                            <div  className='flex items-center gap-2'><div  className='w-6 h-6'>
                                                                <LazyLoadImage
                                                                     className='w-full h-full pointer-events-none mix-blend-multiply'
                                                                    src={asset_url(category.icon || placeholder1_1())}
                                                                    alt={category.name}
                    ...
                    <div  className='lg:col-span-8 xl:col-span-9 space-y-3 md:space-y-0 text-slate-600'>
                        <div  className='w-full h-[100px] md:h-[150px] border'>
                            <LazyLoadImage
                                src={asset_url(childcategories.cover_image || placeholder6_1())}
                        ...
                        <div  className="lg:hidden flex flex-col w-full gap-4 justify-between items-center py-2">
                            <h1  className="font-bold text-xl md:text-2xl w-full">{childcategories.name}</h1>
                        ...
                        <div  className="hidden w-full lg:flex flex-col lg:flex-row gap-2 justify-between items-center py-2">
                            {/* <h1  className="font-bold text-xl md:text-2xl w-full">{childcategories.name}</h1> */}
                            <div  className="border-b pb-3 w-full">
                                <PageTitle title={childcategories.name} />
                            </div>
                        ...
                        {products.meta.total > 0 ? <div>
                            <div  className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-4 gap-2 xl:gap-4">
                                {
