...
export default function FlashDealDetailsPage({ label, slug, products, minimum_price, maximum_price, colors }) {
    ...
    function onSortByChange(e) {
        ...
        router.visit(route('flash_deal_details', { 'slug': slug, 'sort_by': e.target.value, 'min_price': min_price, 'max_price': max_price, 'color': color_name }))
    }

    // filter by price range
    function onInput(e) {
        setRangeValue(e)
        router.visit(route('flash_deal_details', { 'slug': slug, 'sort_by': sort_by, 'min_price': e[0], 'max_price': e[1], 'color': color_name }))
    }
    ...
    function onColorClick(color) {
        if (color_name == color) {
            setSelectedColor('');
            router.visit(route('flash_deal_details', { 'slug': slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price }))
        } else {
            setSelectedColor(color);
            router.visit(route('flash_deal_details', { 'slug': slug, 'sort_by': sort_by, 'min_price': min_price, 'max_price': max_price, 'color': color }))
        }
    }
    ...
    return (
        <DefaultThemeLayout>
            <Head title={"Flash Deal Deatils"} />
            <div className="px-2 py-3 lg:px-2 lg:py-2 xl:px-0">
                {/* Body */}
                <div className='relative lg:grid lg:grid-cols-11 xl:grid-cols-12 gap-3 xl:gap-6 lg:my-5 text-slate-600'>
                    {/* Sidebar for Large Devices */}
                    <div className='hidden lg:block lg:col-span-3 xl:col-span-3 space-y-7'>
                        {/* Categories */}
                    ...
                    <div className='lg:col-span-8 xl:col-span-9 space-y-3 md:space-y-0 text-slate-600'>
                        ...
                        <div className="lg:hidden flex flex-col w-full gap-4 justify-between items-center py-2 border-b">
                            <div className="w-full">
                                <h2 className="font-semibold text-lg w-full">{label}</h2>
                        ...
                        <div className="hidden w-full lg:flex flex-col lg:flex-row gap-2 justify-between items-center border-b pb-1">
                            <div className="w-full">
                                <h2 className="font-semibold text-2xl w-full">{label}</h2>
                                <p className="text-base">{products.meta.total} {products.meta.total > 1 ? 'Items' : 'Item'} Found!</p>
                            </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-4 gap-2 xl:gap-4 pt-2">
                                {/* <ProductCardSkeletons /> */}
                                {
                                    products.data.map((product, i) => (
                                        <Suspense key={i} fallback={<ProductCardSkeletons />}>
                                            <LazyProductCard key={i} product={product} />
                                        </Suspense>
                                    ))
                                }
                            </div>
                        ...
                        </div> : <NothingFound title={'No Flash Deal Product Found!'} />}
                    </div>
                </div>
            </div>
        </DefaultThemeLayout>
