...
export default function Edit({ lang, categoryList, attributeList, attributeValueList, colorsList, brandList, product }) {
    ...
    function handleSubmit(e) {
        ...
        put(route('seller.product.products.update', product.id));
    }
                    ...
                    <div  className="flex items-center">
                        {
                            active_languages.map((language, i) => (
                                ...
                                <a key={i} href={route('seller.product.product_edit', { id: window.btoa(product.id), lang: language.code })}  className={lang == language.code ? 'text-center border py-3 bg-[#3390F3] text-white duration-500 grow' : 'text-center border py-3 hover:bg-[#3390F3] hover:text-white duration-500 grow'}>{language.name}</a>
                            ))
                        }
