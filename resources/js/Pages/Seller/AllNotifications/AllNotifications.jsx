...
const deleteData = ( id ) =>
    {
        router.delete( route( 'seller.notification_destroy', id ) )
    }
                                    ...
                                    <td data-label='Notification' align="left"  className="flex flex-col justify-center items-start">
                                        <div  className="py-1 flex text-start md:items-center gap-1">
                                            { notification.notificationShowDesign != 'only_text' &&
                                                <img  className={ `w-8 md:w-9 h-8 md:h-9 ${ notification.notificationShowDesign == 'design_3' ? 'rounded-full' : notification.notificationShowDesign == 'design_2' ? 'rounded-md' : '' }` } src={ asset_url(notification.notificationType.image) } alt="notification img" />
                                            }
