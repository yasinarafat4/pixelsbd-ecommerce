...
export default function NumberVerification() {
    ...
    const { number_verification } = usePage().props;

    // form functionality
    const { data, setData, post, processing, errors, reset } = useForm({
        identifier: number_verification.identifier,
        sms_body: number_verification.sms_body,
        template_id: number_verification.template_id ?? '',
        status: 1,
    })
        ...
        <div  className='m-4'>
            <form onSubmit={e => handleSubmit(e)}  className='space-y-4'>
                ...
                <div  className="flex justify-end">
                    <button type='submit'  className="bg-sky-500 hover:bg-sky-600 duration-300 py-2 px-4 text-sm rounded text-white">{t('Update Settings')}</button>
                </div>
            </form>
        </div>
    );
