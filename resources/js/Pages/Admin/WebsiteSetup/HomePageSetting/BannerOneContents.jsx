...
export default function BannerOneContents() {

    const { t } = useLaravelReactI18n();
    const { business_settings } = usePage().props

    // const [sliders, setSliders] = useState(business_settings.home_sliders ?? []);
    const [bannerOne, setBannerOne] = useState(business_settings.home_banner_one ?? [
        { 'image': '', 'link': '' },
        { 'image': '', 'link': '' },
        { 'image': '', 'link': '' },
    ]);
    ...
    const handleLinkChange = (event, index) => {
        ...
        let onChangeValue = [...bannerOne];
        onChangeValue[index][name] = value;
        setBannerOne(onChangeValue);
    };

    function onAddFile(v) {
        const Array = [...bannerOne];
    ...
    function handelShowModal(index) {
        if (bannerOne[index].image == "") {
            setSelectedImg([])
        } else {
            setSelectedImg([bannerOne[index].image])
        }
    ...
    function updateHomeBannerOne(params) {
        router.post(route('admin.configuration.update'), {
            types: ['home_banner_one'],
            home_banner_one: bannerOne,
        }, { preserveScroll: true })
    }
        ...
        <div>
            <div  className="container mt-4">
                <div  className="p-4 rounded border bg-slate-100 shadow">
                    <h1  className="text-lg font-semibold mb-2">Banner One Contents</h1>
            ...
            <div  className="flex justify-end my-4">
                <button onClick={updateHomeBannerOne}  className="bg-[#008FE1] duration-300 py-2 px-20 rounded-md text-white text-md">{t('Save')}</button>
            </div>
        </div>
    )
