...
export default function BannerThreeContents() {
    const { t } = useLaravelReactI18n();
    const { business_settings } = usePage().props

    // const [sliders, setSliders] = useState(business_settings.home_sliders ?? []);
    const [bannerThree, setBannerThree] = useState(business_settings.home_banner_three ?? [
        { 'image': '', 'link': '' },
        { 'image': '', 'link': '' },
        { 'image': '', 'link': '' },
    ]);
    ...
    const handleLinkChange = (event, index) => {
        ...
        let onChangeValue = [...bannerThree];
        onChangeValue[index][name] = value;
        setBannerThree(onChangeValue);
    };

    function onAddFile(v) {
        const Array = [...bannerThree];
        Array[currentIndex].image = v[0]
        setBannerThree(Array)
        closeModal();
    }
    ...
    function handelShowModal(index) {
        if (bannerThree[index].image == "") {
            setSelectedImg([])
        } else {
            setSelectedImg([bannerThree[index].image])
        }
    ...
    function updateHomeBannerThree(params) {
        router.post(route('admin.configuration.update'), {
            types: ['home_banner_three'],
            home_banner_three: bannerThree,
        }, { preserveScroll: true })
    }
        ...
        <div>
            <div  className="container mt-4">
                <div  className="p-4 rounded border bg-slate-100 shadow">
                    <h1  className="text-lg font-semibold">Banner Three Contents</h1>
            ...
            <div  className="flex justify-end my-4">
                <button onClick={updateHomeBannerThree}  className="bg-[#008FE1] duration-300 py-2 px-20 rounded-md text-white text-md">{t('Save')}</button>
            </div>
        </div>
    )
