...
export default function Edit() {
    ...
    const { default_currency_symbol, products, flashDeal, flashDealProductId } = usePage().props;
    ...
    function handleSubmit(e) {
        e.preventDefault()
        put(route('admin.marketing.flashdeal.update', flashDeal.id))
    }

    // Showing selected products and list
    useEffect(() => {
        const filterProductList = products.filter(item => flashDealProductId.includes(item.id))
        let p = [];
        filterProductList.forEach(element => {
            p.push({ label: element.name, value: element.id });
        });
        setSelectedProductList(filterProductList);
        setSelectedProducts(p)

        const value = p.map(item => item.value);
        setData('products', value)
    }, [])

    // Showing selected discount & discount type
    useEffect(() => {
        flashDeal.flash_deal_products.forEach(ele => {
            setData(prevData => ({ ...prevData, ['discount_' + ele.product_id]: ele.discount }));
            setData(prevData => ({ ...prevData, ['discount_type_' + ele.product_id]: ele.discount_type }));
        })
    }, [])
    ...
    function onProductChange(e) {
        ...
        e.map((item) => {
            ...
            setData(prevData => ({ ...prevData, ['discount_type_' + item.value]: 'percent' }));
        })
            ...
            <div  className='p-4'>
                ...
                <div  className='card rounded-lg shadow bg-white border-[1px] border-slate-200 py-5 max-w-7xl mx-auto'>
                    <div  className="flex items-center justify-between border-b pb-3 px-6">
                        <div>
                            <h2  className="text-lg font-medium text-slate-600">{t('Update Flash Deal Information')}</h2>
                        </div>
                    </div>
                                        ...
                                        <div  className="w-full">
                                            ...
                                            <div  className="flex items-center gap-3">
                                                {data.banner && <div  className="relative">
                                                    ...
                                                    <img  className='w-32 h-32 border rounded-xl p-3 mt-3' src={data.banner} alt={'Banner'} />
                                                </div>}
                                            </div>
                                        </div>
