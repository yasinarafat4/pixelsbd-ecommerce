...
export default function Index ( { states, countries } )
{
    ...
    {
        let selected = [];
        let filter = countries.filter( ( country ) => country.name == queryParams.country )
        filter.map( ( item ) =>
        {
            selected.push( { value: item.id, label: item.name } )
        } )
        setSelectedCountry( selected[ 0 ] )
        setData( 'country_id', selected[ 0 ]?.value )
    }, [] )
    ...
    {
        e.preventDefault()
        post( route( 'admin.configuration.shipping.state.store' ),
            {
                onSuccess: () => { reset() }
            } )
    }

    function OnStateStatusChange ( e, id )
    {
        router.put( route( 'admin.configuration.shipping.state.status', id ), {
    ...
    {
        router.visit( route( 'admin.configuration.shipping.state.index' ), {
            data: {
                search: search,
    ...
    const deleteData = ( id ) =>
    {
        router.delete( route( 'admin.configuration.shipping.state.destroy', id ) )
    }
                        ...
                        <ul>
                            ...
                            <li>
                                <span  className="inline-flex gap-1 items-center">
                                    <FaHouseFlag  className="text-base text-slate-900" />
                                    <span>States</span>
                                </span>
                            </li>
                        </ul>
                ...
                <div  className="flex items-start gap-2">
                    ...
                    <div  className='w-8/12 card rounded-lg shadow bg-white border-[1px] border-slate-200 py-3'>
                        <div  className="grid grid-cols-12 border-b pb-3 px-6 gap-3">
                            <div  className="col-span-2">
                                <h2  className="text-lg font-medium text-slate-600">{ t( 'All States' ) }</h2>
                            </div>
                            {/* Search and Filter*/ }
                            { states.data.length > 0 && <div  className="w-full col-span-4">
                            ...
                            { states.data.length > 0 && <div  className="flex items-center gap-1 w-full col-span-6">
                                <Select
                                    name="state"
                        ...
                        <div  className='card-body'>
                            { states.data.length > 0 ? <div>
                                    ...
                                    <tbody>
                                        ...
                                        { states.data.map( ( state, i ) => (
                                            <tr key={ i }  className='text-slate-600 border-b'>
                                                <td align="text-sm left">{ ( i + 1 ) + ( ( states.current_page - 1 ) * ( states.per_page ) ) }</td>
                                                <td align="text-sm left">{ state.name }</td>
                                                <td align="text-sm left">{ state.country.name }</td>
                                                <td align="text-sm center">
                                                    <input type="checkbox" onChange={ ( e ) => OnStateStatusChange( e, state.id ) } checked={ state.status == 1 }  className="toggle toggle-sm toggle-success" />
                                                </td>
                                                <td align="center"  className="space-x-2">
                                                    <Link href={ route( 'admin.configuration.shipping.state.edit', state.id ) } method="get" data={ queryParams }>
                                                    ...
                                                    <Link><div onClick={ e => handleDelete( state.id ) } data-tip={ t( 'Delete' ) }  className="tooltip cursor-pointer p-[10px] text-red-600 hover:text-slate-200 bg-red-100 hover:bg-red-600 duration-500 rounded-full">
                                ...
                                <div  className="m-4">
                                    <p  className='text-slate-600 text-sm'>Showing { states.from || 0 } to { states.to || 0 } of { states.total }</p>
                    ...
                    < div  className='w-4/12 card rounded-lg shadow bg-white border-[1px] border-slate-200' >
                        <div  className="flex items-center justify-start border-b py-3 px-4">
                            <div>
                                <h2  className="text-lg font-medium text-slate-600">{ t( 'Add New State' ) }</h2>
                            </div>
                        </div>
                        ...
                        <form onSubmit={ handleSubmit }  className="space-y-3 py-4 px-4">
                            ...
                            <div  className="flex flex-col">
                                ...
                                <Select
                                    ...
                                    options={ countries.map( country => ( { value: country.id, label: country.name } ) ) }
                                />
                                { errors.country_id && <div  className="text-red-500 text-sm mt-1">Select country</div> }
                            </div>
                            <div  className="flex justify-end">
