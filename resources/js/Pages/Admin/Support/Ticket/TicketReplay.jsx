...
export default function TicketReplay() {
    ...
    function onStatusChange(e, id) {
        router.put(route('admin.support.ticket_status', id), {
            status: e.value
        });
    }
    ...
    function handleSubmit(e) {
        e.preventDefault()
        post(route('admin.support.ticket_replay.store'), {
            onSuccess: () => reset()
        })
    }
        ...
        <AdminLayout>
            ...
            <div  className='p-4'>
                    ...
                    <div  className="text-sm breadcrumbs text-slate-600">
                        <ul>
                            <li  className="inline-flex gap-1 items-center">
                                ...
                                <Link href={route('admin.support.tickets.index')}>{t('Ticket')}</Link>
                            </li>
                            <li>
                ...
                <div  className="space-y-6 mb-10">
                    {/* Replay Form */}
                    <div  className='card rounded-lg shadow bg-slate-50 border-[1px] border-slate-200 py-5 max-w-5xl mx-auto'>
                        <div  className="grid grid-cols-5 items-center justify-between border-b pb-3 px-4">
                            {/* Ticket Info */}
                            <div  className="col-span-3 flex flex-col items-start pb-3 gap-2">
                                <div  className="flex items-center gap-3">
                                    <p  className="text-xl font-medium">{ticket.subject}</p>
                                    <p  className="text-xl font-medium">#{ticket.code}</p>
                                </div>
                                <div  className="flex items-center gap-4">
                                    <h2  className="text-sm font-medium">{ticket.user.name}</h2>
                                    <p  className="text-xs">{moment().format('ll')}</p>
                                    <div  className="badge badge-ghost bg-slate-500 font-bold text-white text-xs">{toCamel(ticket.status)}</div>
                                </div>
                            </div>
                            {/* Status */}
                            <div  className="col-span-2 w-full">
                                <label  className='label-text text-slate-600 text-sm font-medium' htmlFor="status">{t('Change Status')}</label>
                                <Select
                                    name="status"
                                    placeholder="Select Status"
                                     className="text-sm w-full rounded"
                                    classNamePrefix="react-select"
                                    value={status.find(option => option.value == ticket.status)}
                                    onChange={e => onStatusChange(e, ticket.id)}
                                    options={status}
                                />
                            </div>
                        </div>
                    ...
                    <div  className='card rounded-lg shadow bg-white border-[1px] border-slate-200 max-w-5xl mx-auto'>
                        ...
                        <div  className="flex justify-between px-4 items-center border-b border-slate-200">
                            <div  className="py-2 flex items-center gap-2">
                                <img  className="w-9 h-9 rounded-full" src={asset_url(ticket.user.image || placeholder_user())} alt={ticket.user.name} />
                                <div>
                                    <h2  className="text-base font-semibold">{ticket.user.name}</h2>
                                    <p  className="text-xs">{moment(ticket.created_at).format('lll')}</p>
                                </div>
                            </div>
                        </div>
                        <div  className="p-4 space-y-2">
                            ...
                            {/* File */}
                            {
                                ticket.files && ticket.files.split(',').map((file, i) => {
                    ...
                    {/* Replay */}
                    {
                        ...
                        ticket.ticket_replay.map((replay, i) => {

                            return <div key={i}>
                                {
                                    ticket.user.id == replay.user.id ?
                                            ...
                                            <div  className="p-4 space-y-2">
                                                ...
                                                {/* File */}
                                                {
                                                    replay.files && replay.files.split(',').map((file, i) => {
                                                        return <div key={i}  className="flex flex-col badge bg-[#DAE0E5] text-slate-600 text-[11px] rounded">
                                                            <a  className="flex items-center gap-2" target="_blank" rel="noreferrer" href={'/storage/uploads/ticket_images/' + file}><ImDownload2  className="text-sm" />{file.slice(-30)}</a>
                                                        </div>
                                                    })
                                                }
                                            </div>
