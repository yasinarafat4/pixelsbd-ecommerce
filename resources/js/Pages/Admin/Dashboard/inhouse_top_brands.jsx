...
export default function InhouseTopBrands ()
{
    ...
    {
        axios.post( route( 'admin.dashboard.inhouse_top_brands' ), { 'interval_type': type }, { responseType: "json" } )
            .then( response =>
            {
                setInhouseTopBrands( response.data )
                setLoading( false )
            } )
    }
    return (
        <div  className="self-start h-full p-5 bg-white drop-shadow-lg border border-slate-200 rounded-lg space-y-5">
            <div>
                <h2  className="text-base text-[#F0416C] font-medium">{ t( 'In-house Top Brands' ) }</h2>
            </div>
            <div  className='h-full space-y-2'>
                ...
                { loading ?
                    ...
                    : <table  className="table">
                        <tbody>
                            { inhouseTopBrands?.map( ( inhouseTopBrand, i ) => (
                                    ...
                                    <td>
                                        <div  className="avatar">
                                            <div  className="mask mask-squircle h-12 w-12">
                                                <img
                                                    src={ asset_url(inhouseTopBrand.logo || placeholder1_1()) }
                                                    alt="Avatar Tailwind CSS Component" />
                                            </div>
                                        </div>
                                    </td>
                                    <td>
                                        <div  className="font-bold">{ JSON.parse( inhouseTopBrand.name )[ active_locale ] }
                                        </div>
                                    </td>
                                    <td  className='text-[#F0416C] font-bold'>{ currencyFormat( inhouseTopBrand.total ) }</td>
                                </tr>
                            ) ) }
                        </tbody>
                    </table> }
