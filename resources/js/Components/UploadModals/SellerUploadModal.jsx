...
export default function SellerUploadModal({ limit = 1, onAddFile, closeModal, showModal, selected }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (previewCanvasRef.current) {
            previewCanvasRef.current.toBlob(async (blob) => {
                ...
                router.post(route('seller.image_upload_modal'), formData, {
                    onStart: () => {
                        setProgress(true)
                    },
                    onFinish: () => {
                ...
                <div className="text-slate-600">
                    ...
                    <div className="flex justify-between items-center py-3 px-6 bg-[#F2F3F8]">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => handleTabChange("selectFile")}
                            ...
                            <button onClick={() => handleTabChange("uploadFile")}
                                ...
                                <div className="px-6 py-3 grid grid-cols-5 gap-2 w-full overflow-y-scroll">
                                    ...
                                    {imageList.data && imageList.data.map((media, i) => (
                                        <div key={i} className={`overflow-hidden rounded-lg shadow-lg cursor-pointer ${selectedImage.some((img) => img === media.url) ? 'ring-[3px] ring-blue-500' : ''}`} onClick={() => onImageSelect(media)}>
                                            {/* <img  className="w-full h-32 object-cover transform hover:scale-105 transition-transform duration-300" src={media.url} alt="Image 1" /> */}
                    ...
                    {/* Modal Bottom */}
                    {activeTab === "selectFile" && <div className="flex justify-between items-end py-3 px-6 bg-[#F2F3F8]">
